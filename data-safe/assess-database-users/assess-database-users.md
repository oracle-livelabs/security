...
4. Rename the latest user assessment: Click the pencil icon to the right of **Name**, enter **UA_target-database1** (replace **target-database** with the name of your target database), and click the **Save** icon.
...
3. Name the assessment **UA_target-database2** (replace **target-database** with the name of your target database), and click **Refresh now**. Wait for the status of the latest user assessment to read as **SUCCEEDED**. Oracle Data Safe automatically saves a static copy of the assessment to the Assessment History.
...
4. From the **Select assessment** drop-down list, select the initial assessment for your target database (named **UA-target-database1**). As soon as you select it, the comparison operation is started.
